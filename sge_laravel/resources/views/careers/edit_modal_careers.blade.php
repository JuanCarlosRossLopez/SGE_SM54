<div id="edit{{ $career->id }}"
    ...
    <form method="POST" action="{{ url('carreras', $career->id) }}">
        @method('PUT')
        @csrf
            ...
            <div class="bg-white p-2">
                <div class="modal-body flex-row gap-4 mb-0 overflow-y-auto flex items-center justify-center p-10">
                    <div class="flex flex-col items-center justify-center">
                        ...
                        <form action="{{ route('carreras.update', $career->id) }}" method="POST"
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="text" name="career_name" id="career_name"
                                        placeholder="Nombre de la carrera" value="{{ $career->career_name }}"
                                        class="rounded-md border border-gray-300 p-2 w-full">
                                </div>
                                <div>
                                    <label for="career_description" class="block text-gray-700">Descripción de la
                                        carrera</label>
                                ...
                                <select id="division_id" name="division_id"
                                    ...
                                    @foreach ($divisions as $division)
                                        <option value="{{ $division->id }}"
                                            {{ $career->division_id == $division->id ? 'selected' : '' }}>
