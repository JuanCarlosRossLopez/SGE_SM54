<div id="check{{$book->id}}" class="modalCheck h-screen w-full fixed left-0 top-0 hidden flex justify-center items-center bg-black bg-opacity-50">
        ...
        <div class="bg-white p-2">
            <div class="modal-body flex-col gap-4 mb-0 overflow-y-auto flex items-center justify-center P-10 ">
                <h1 class="poppins-light text-[20px]">
                    ¿Desea aceptar este libro?
                </h1>
                <div class="flex flex-row gap-4 text-white ">
                    <form action="{{route('gestion_libros.update', $book->id)}} " method="POST">
                        @csrf
                        @method('PATCH')
                        <input type="hidden" name="id" value="{{ $book->id }}">
                        <button class="bg-green-500 rounded w-20 h-12 text-white" type="submit">
                            Aceptar
                        </button>
                    </form>
