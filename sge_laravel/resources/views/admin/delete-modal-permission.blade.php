...
<div id="delete_permission{{$permission->id}}" class="delete-modal-permissions h-screen w-full fixed left-0 top-0 hidden flex justify-center items-center bg-black bg-opacity-50">
    <form action="{{route('roles.delete_permission', $permission->id)}}" method="POST">
    @method('DELETE')
    @csrf
    <input type="hidden" name="role_id" value="{{$role->id}}"> <!-- Incluimos el ID del rol en un campo oculto -->
    <div class="bg-[#01A080] w-full rounded shadow-lg max-w-2xl">
        <div class="border-b px-4 py-2 flex justify-between items-center">
            <h3 class="font-semibold text-lg ml-60 text-white">Elimar  roles</h3>
