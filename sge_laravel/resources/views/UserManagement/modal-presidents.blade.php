<div id="edit{{ $user->id }}"
    ...
    <div class="bg-[#01A080] w-full rounded shadow-lg max-w-sm">
        <div class="border-b px-4 py-2 flex justify-between items-center">
            <h3 class="font-semibold text-lg text-white text-center flex-grow">Editar presidente</h3>
                ...
                <form action="{{ url('usuarios', $user->id) }}" method="POST" class="flex flex-col items-center">
                    ...
                    <div class="mb-4">
                        <label class="block text-gray-700 text-sm font-bold mb-2">Nombre
                            del
                        presidente</label>
                        <input type="text" name="name" value="{{ $presidencies->presidencies_name}}" class="rounded input-field">
                    </div>
                    <div class="mb-4">
                        <label class="block text-gray-700 text-sm font-bold mb-2">Nomina</label>
                        <input type="text" name="text" value="{{ $presidencies->payroll_presidencies }}" class="rounded input-field">
                    </div>
                    <div class="mb-4">
                        <label class="block text-gray-700 text-sm font-bold mb-2">Division</label>
                        <input type="text" name="text" value="{{ $presidencies->Division }}" class="rounded input-field">
                    </div>

                    <div class="mb-4">
                        <label class="block text-gray-700 text-sm font-bold mb-2">Carrera</label>
                        <input type="text" name="text" value="{{ $presidencies->career }}"
    ...
    <div class="bg-white w-auto p-4 rounded-lg">
        <div class="flex flex-col justify-between items-center">
            <div class="flex flex-row gap-10 ">
                <h3 class="font-semibold text-lg text-gray-800">Datos del presidente:
                    {{ $user->name }}</h3>
            ...
            <div class="py-4">
                <h5 class="py-2">Nombre de Usuario: {{ $presidencies->presidencies_name }} </h5>
                <h5 class="py-2">Nomina: {{ $presidencies->payroll_presidencies }} </h5>
...
<div class="modal5 h-screen w-full fixed left-0 top-0 hidden flex justify-center items-center bg-black bg-opacity-50">
    <div class="bg-white w-96 p-4 rounded-lg">
        <div class="flex justify-between items-center">
            <h3 class="font-semibold text-lg text-gray-800">presidente agregado correctamente</h3>
...
<div class="modal6 h-screen w-full fixed left-0 top-0 hidden flex justify-center items-center bg-black bg-opacity-50">
    <div class="bg-white w-96 p-4 rounded-lg">
        <div class="flex justify-between items-center">
            <h3 class="font-semibold text-lg text-gray-800">presidente editado correctamente</h3>
