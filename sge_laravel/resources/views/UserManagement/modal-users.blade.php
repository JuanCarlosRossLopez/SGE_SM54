<div idModal="edit_{{ $user->id }}" class="modal h-screen w-full fixed left-0 top-0 hidden flex justify-center items-center bg-black bg-opacity-50 ">
...
<div idModal="Show_User_{{ $user->id }}"
    class="modal h-screen fixed left-0 top-0 hidden flex justify-center items-center bg-black bg-opacity-50">
    <div class="bg-[#01A080] rounded shadow-lg max-w-2xl">
        <div class="border-b px-4 py-2 flex justify-between items-center">
            <h3 class="font-semibold text-lg ml-60 text-white">Datos del usuario:
                {{ $user->name }}</h3>
            <button class="close-modal bg-white rounded-full h-[1rem] flex items-center">
        ...
        <div class="bg-white p-2">
            <div class="modal-body ">
                <div class="py-6 flex flex-col px-2 text-center gap-4">
                    <div class="flex flex-row gap-2">
                        <h1 class=" font-sans font-semibold text-[#545454]">Nombre del usuario:</h1>
                        <h5> {{ $user->name }} </h5>
                    </div>
                    <div class="flex flex-row gap-2">
                        <h1 class=" font-sans font-semibold text-[#545454]">Email:</h1>
                        <h5> {{ $user->email }} </h5>
                    </div>
                </div>
            </div>
...
<div idModal="delete_{{ $user->id }}"
    ...
    <div class="bg-[#01A080] w-full rounded shadow-lg max-w-2xl ">
        <div class="px-4 py-2 flex items-center justify-center">
            <h3 class="font-semibold text-lg text-white ">¿Realmente desea eliminar al usuario:
                {{ $user->username }}?</h3>
        ...
        <div class="bg-white p-2">
            ...
            <div class="modal-body flex items-center justify-center gap-3">
                <form action="{{ url('usuarios', $user->id) }}" method="POST">
                    @csrf
                    @method('DELETE')
                    <button
                        class="close-modal bg-emerald-600 hover:bg-emerald-500 rounded-lg px-5 py-3 font-sans font-semibold text-[#000000]">
                        Si
                    </button>
                </form>
                <button class="close-modal bg-red-600 hover:bg-red-500 rounded-lg px-5 py-3 font-sans font-semibold text-[#00000]">
                    No
                </button>
            </div>
        </div>
